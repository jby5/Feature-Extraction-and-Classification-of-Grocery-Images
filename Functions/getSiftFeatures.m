function [features,metrics]=getSiftFeatures(img)
[~,~,ColorChannel] = size(img);
if ColorChannel > 1
    img = rgb2gray(img);
end
img = double(img)/255;
ScaleSpaceNum = 2; % number of scale space intervals
...
%% JY - Convert SIFT into a keypoint x feature vector matrix
features=[];
flattenedCell=[];
for i=1:OctaveNum
    for j=1:StackNum-3
        flattenedCell=reshape(SIFT{i,j},[],128);
        features=[features; flattenedCell];
